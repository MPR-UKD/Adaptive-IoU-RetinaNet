...
def test_change_box_order():
    ...
    assert (change_box_order(xyxy, "xyxy2xywh") == xywh).all()
    assert (change_box_order(xywh, "xywh2xyxy") == xyxy).all()
...
def test_one_hot_embedding():
    ...
    one_hot = one_hot_embedding(labels, 3)
...
def test_box_nms():
    ...
    assert box_nms(bboxes, scores, 0.3)[0] == 0
    assert box_nms(bboxes, scores, 0.3)[1] == 2
